...
request
  .get("http://api.tvmaze.com/search/shows?q=Narcos")
  .then(function(response) {
    const peliNarcos = response.body;
    var sscore = peliNarcos[0].score;
    answerElement_tvmaze_1.innerHTML = sscore;
  });
//========================================================================
...
request.get("http://api.tvmaze.com/shows/83/episodes").then(function(response) {
  const episodSimpsons = response.body;
  // console.log(episodSimpsons)
  var episodes = episodSimpsons.length;
  answerElement_tvmaze_2.innerHTML = episodes;
});
...
request
  ...
  .then(function(response) {
    const actorAndrew = response.body;
    // console.log(actorAndrew);
    var credits = actorAndrew.length;
    answerElement_tvmaze_4.innerHTML = credits;
  });
...
request.get("http://api.tvmaze.com/shows/431/seasons").then(function(response) {
  const showFriends = response.body;
  // console.log(showFriends);
  var premiereDate = showFriends[8].premiereDate;
  answerElement_tvmaze_3.innerHTML = premiereDate;
});
